...
exports.parseExplorePage = (context)=>{
  const result = {
    content: [],
    genres: []
  };

  let tabs = utils.fv(context, 'contents:singleColumnBrowseResultsRenderer:tabs');

  if(Array.isArray(tabs)){
    tabs.forEach((el)=>{
      if("tabRenderer" in el){
        let content = utils.fv(el, 'tabRenderer:content:sectionListRenderer:contents');
        if(Array.isArray(content)){
          content.forEach((section)=>{
            //console.log(section);
            if("musicNavigationButtonRenderer" in section){
              let button = section['musicNavigationButtonRenderer'];
              let title = utils.fv(button, 'buttonText:runs');
              title = Array.isArray(title) ? title[0]["text"] : "";

              let color = utils.kN(utils.fv(button, 'solid:leftStripeColor')) || "#000000";

              let clickCommand = button["clickCommand"];

              result.genres.push({
                title: title,
                color: color,
                browse: {
                  browseEndpoint: clickCommand["browseEndpoint"],
                  browseId: clickCommand["browseEndpoint"]["browseId"],
                  clickTrackingParams: clickCommand["clickTrackingParams"],
                  trackingParams: button['trackingParams'],
                  params: clickCommand["browseEndpoint"]["params"]
                }
              });
            }else if("musicTwoRowItemRenderer" in section){
              let music = section['musicTwoRowItemRenderer'];

              if(music.aspectRatio === "MUSIC_TWO_ROW_ITEM_THUMBNAIL_ASPECT_RATIO_SQUARE"){
                let title = utils.fv(music, 'title:runs');
                title = Array.isArray(title) ? title[0]["text"] : "";

                let subtitle = utils.fv(music, 'subtitle:runs');
                subtitle = Array.isArray(subtitle) ? subtitle.map((v)=>{return v["text"]}).join("") : "";

                let thumbnails = utils.fv(music, 'thumbnailRenderer:musicThumbnailRenderer:thumbnail:thumbnails');
                ...
                result.content.push({
                  ...
                  browse: {
                    ...
                    trackingParams: music['trackingParams'],
                  }
                });
              }
            }
...
exports.parseSearchResult = (context) => {
  ...
  sectionList.forEach((sectionContext) => {
    const flexColumn = _.concat(
      utils.fv(sectionContext, 'musicResponsiveListItemFlexColumnRenderer')
    );
    ...
    switch (type) {
      case 'Song':
        result.content.push(
          Object.freeze({
            type: _.lowerCase(
              _.nth(utils.fv(_.nth(flexColumn, 1), 'runs:text'), 0)
            ),
            ...
            duration: utils.hms2ms(
              _.nth(utils.fv(_.nth(flexColumn, 1), 'runs:text'), 6)
            ),
            thumbnails: utils.fv(
              sectionContext,
      ...
      case 'Playlist':
        result.content.push(
          Object.freeze({
            ...
            count: _.toNumber(
              _.nth(
                _.words(_.nth(utils.fv(_.nth(flexColumn, 1), 'runs:text'), 5)),
                0
              )
            ),
            thumbnails: utils.fv(
  ...
  sectionList.forEach((sectionContext) => {
    ...
    result.content.push(
      Object.freeze({
        ...
        artist: _.join(
          _.filter(
            utils.fv(_.nth(flexColumn, 1), 'runs:text').slice(1, -1),
            (v) => ' • ' != v && true
          ),
          ''
        ),
    ...
    result.content.push(
      Object.freeze({
        ...
        trackCount: _.toNumber(
          _.nth(_.words(_.last(utils.fv(_.nth(flexColumn, 1), 'runs:text'))), 0)
        ),
        thumbnails: utils.fv(
          sectionContext,
          'musicThumbnailRenderer:thumbnails'
...
exports.parseArtistPage = (context) => {
  ...
  const descriptionContext = utils.fv(context, 'musicDescriptionShelfRenderer');
  if (!Array.isArray(descriptionContext)) {
    ...
    result.views = _.parseInt(
      _.replace(
        _.nth(
          _.split(utils.fv(_.at(descriptionContext, 'subheader'), 'text'), ' '),
          0
        ),
        /,/g,
        ''
  ...
  utils
    ...
    .forEach((itemContext) => {
      const flexColumn = _.concat(
        utils.fv(itemContext, 'musicResponsiveListItemFlexColumnRenderer')
      );
        ...
        for (let i = 0; i < itemContext.length; i++) {
          switch (carouselName) {
            ...
            case 'albums':
              result.products[carouselName].content.push({
                type: _.nth(
                  utils.fv(_.at(itemContext[i], 'subtitle'), 'text'),
                  0
                ),
                browseId: utils.fv(
                  _.at(itemContext[i], 'navigationEndpoint'),
        ...
        switch (carouselName) {
          ...
          case 'videos':
            result.products[carouselName].content.push({
              ...
              name: utils.fv(_.at(itemContext, 'title'), 'text'),
              author: _.join(
                _.dropRight(utils.fv(_.at(itemContext, 'subtitle'), 'text'), 2),
                ''
              ),
...
exports.parsePlaylistPage = (context) => {
  ...
  if (!_.has(context, 'continuationContents')) {
    ...
    result.trackCount = parseInt(
      _.words(
        _.nth(utils.fv(_.at(pageHeader, 'secondSubtitle'), 'runs:text'), 0)
      )
    );
...
exports.parseAlbumPage = (context) => {
  ...
  let albumRelease = utils.fv(context, 'musicAlbumRelease');
  let playlistId = utils.fv(context, 'playlistId');

  if(playlistId instanceof Array){
    playlistId = playlistId.filter(function(item, pos, self){
  ...
  if(!((albumArtist instanceof Array || albumArtist instanceof Object) && Object.keys(albumRelease).length > 0)){
    albumRelease = utils.fv(context, 'musicDetailHeaderRenderer');

    result.title = utils.fv(_.at(albumRelease, 'title'), 'runs:text');
    result.description = utils.fv(_.at(albumRelease, 'subtitle'), 'runs:text').join("");
    result.thumbnails = utils.fv(albumRelease, 'thumbnail:thumbnails');

    const albumTrack = utils.fv(context, 'musicShelfRenderer:contents');

    if (albumTrack instanceof Array) {
      result.tracks = albumTrack.map((t)=>{
        return {
    ...
    //console.log(JSON.stringify(albumTrack, null, 2));
...
exports.parseAutomixPlaylist = (context) => {
  const result = {
    clickTrackingParams: '',
    playlistId: '',
    params: '',
  ...
  const automixPlaylist = utils.fv(panelContext, 'automixPlaylistVideoRenderer');

  result['clickTrackingParams'] = _.nth(_.at(automixPlaylist, 'navigationEndpoint.clickTrackingParams'), 0);

  result['playlistId'] = _.nth(_.at(automixPlaylist, 'navigationEndpoint.watchPlaylistEndpoint.playlistId'), 0);

  result['params'] = _.nth(_.at(automixPlaylist, 'navigationEndpoint.watchPlaylistEndpoint.params'), 0);

  result['trackingParams'] = _.nth(_.at(automixPlaylist, 'trackingParams'), 0);
...
exports.parseNextPanel = (context) => {
  ...
  const panelVideoRenderer = utils.fv(panelContext, 'playlistPanelVideoRenderer');

  //console.log(JSON.stringify(panelVideoRenderer, null, 2));
  ...
  utils
    ...
    .forEach((itemContext) => {
      result.content.push({
        ...
        title: utils.fv(_.at(itemContext, 'title'), 'runs:text'),
        subtitle: _.join(_.dropRight(utils.fv(_.at(itemContext, 'longBylineText'), 'text'), 2), ''),
        thumbnails: utils.fv(itemContext, 'thumbnail:thumbnails'),
        playlistId: _.nth(
          _.at(itemContext, 'navigationEndpoint.watchEndpoint.playlistId'),
          0
        ),
